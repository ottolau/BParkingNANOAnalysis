...
class BParkingNANOAnalyzer(object):
  def __init__(self, inputfiles, outputfile, inputbranches, outputbranches):
  ...
  def init_output(self):
    print('[BParkingNANOAnalyzer::init_output] INFO: FILE: {}/{}. Initializing output {}...'.format(self._ifile+1, self._num_files, 'tree'))
    if os.path.isfile(self._file_out_name+'.root'): os.system('rm {}'.format(self._file_out_name+'.root'))

  def fill_output(self):
    print('[BParkingNANOAnalyzer::fill_output] INFO: FILE: {}/{}. Filling the output {}...'.format(self._ifile+1, self._num_files, 'tree'))
    self._branches = self._branches[self._outputbranches].sort_index(axis=1)
