...
if __name__ == "__main__":
  ...
  for mvaCut in mvaCutList:
    ...
    S, SErr, B = fit_unbinned.fit(tree, outputfile + '_mva_{0:.3f}'.format(mvaCut).replace('.','-') + '.pdf', doPartial=True, drawSNR=True, mvaCut=mvaCut)
    #S, SErr, B = fit_unbinned.fit(tree, outputfile + '_mva_{0:.3f}'.format(mvaCut).replace('.','-') + '.pdf', isMC=True, drawSNR=True, mvaCut=mvaCut)
